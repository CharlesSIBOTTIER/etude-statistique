...
for (j in 1:nrow(tableEnfNonAcc)){  
  if(is.na(tableEnfNonAcc[j,805])){tableEnfNonAcc[j,811]<-0}
}

#Retraitement du revenu annuel
for (j in 1:nrow(tableEnfNonAcc)){  
  if(is.na(tableEnfNonAcc[j,811])){tableEnfNonAcc[j,811]<-0}
}
...
for (j in 1:nrow(tableEnfNonAcc)){  
  if(is.na(tableEnfNonAcc[j,896])){tableEnfNonAcc[j,896]<-0}
}

#Retraitement sur le statut d'occupation du logement
for (j in 1:nrow(tableEnfNonAcc)){  
  if(is.na(tableEnfNonAcc[j,898])){tableEnfNonAcc[j,898]<-0}
}
...
for(j in 1:nrow(tableEnfNonAcc)){
  if(is.na(tableEnfNonAcc[j,1030])){tableEnfNonAcc[j,1030]<-0}
}

#Cuisine protection enfant
for(j in 1:nrow(tableEnfNonAcc)){
  if(is.na(tableEnfNonAcc[j,1035])){tableEnfNonAcc[j,1035]<-0}
}
