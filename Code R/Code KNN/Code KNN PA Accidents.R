...
for (i in 630:633){
  for (j in 1:nrow(tablePaACC)){  
    if(is.na(tablePaACC[j,i])){tablePaACC[j,i]<-"4"}
    if(tablePaACC[j,i]=="NA"){tablePaACC[j,i]<-"4"}
  }
  tablePaACC[,i]<-as.numeric(tablePaACC[,i])
}

for (i in 635:635){
  for (j in 1:nrow(tablePaACC)){  
    if(is.na(tablePaACC[j,i])){tablePaACC[j,i]<-"12"}
    if(tablePaACC[j,i]=="NA"){tablePaACC[j,i]<-"12"}
  }
  tablePaACC[,i]<-as.numeric(tablePaACC[,i])
}
...
for (i in 643:643){
  for (j in 1:nrow(tablePaACC)){  
    if(is.na(tablePaACC[j,i])){tablePaACC[j,i]<-"6"}
    if(tablePaACC[j,i]=="NA"){tablePaACC[j,i]<-"6"}
    if(tablePaACC[j,i]=="0"){tablePaACC[j,i]<-"98"}
  }
  tablePaACC[,i]<-as.numeric(tablePaACC[,i])
}
...
tablePaACC[,5]<-as.factor(tablePaACC[,5])



# Splitting data into train
# and test data
split <- sample.split(tablePaACC, SplitRatio = 0.7)
train_cl <- subset(tablePaACC, split == "TRUE")
test_cl <- subset(tablePaACC, split == "FALSE")
...
tablePaACC[,5]<-as.numeric(tablePaACC[,5])
